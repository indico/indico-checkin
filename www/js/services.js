...
angular.module('Checkinapp.services', [])
    ...
    .service('OAuth', function () {
        ...
        function deleteEvent(server_id, event_id) {
            ...
            localStorage.setItem('events', JSON.stringify(events));
            // remove registrations cache from local storage
            var cacheData = JSON.parse(localStorage.getItem('registrantsCache') || "{}");
            delete cacheData[server_id][event_id];
            localStorage.setItem('registrantsCache', JSON.stringify(cacheData));
            return true;
        }
        ...
        function getRegistrantsForEvent(server_id, event_id, callback) {
            if(getServer(server_id).is_oauth2) {
                ...
                hello('indico').api('/api/events/' + event_id + '/registrants')
                    .then(function (data) {
                        var cacheData = JSON.parse(localStorage.getItem('registrantsCache') || "{}");
                        if (!cacheData[server_id]) cacheData[server_id] = {};
                        cacheData[server_id][event_id] = data.registrants;
                        localStorage.setItem('registrantsCache', JSON.stringify(cacheData));
                        callback(data);
                    }, function (data) {
        ...
        function getRegistrantsForEventFromCache(server_id, event_id) {
            var cacheData = JSON.parse(localStorage.getItem('registrantsCache') || "{}");
            if (cacheData[server_id] && cacheData[server_id][event_id]) {
                return cacheData[server_id][event_id]
            }
            return [];
        }
            ...
            if(getServer(server_id).is_oauth2) {
                ...
                .then(function (data) {
                    ...
                    if(checkin_secret == data.checkin_secret) {
                        ...
                        showAlert("Invalid Ticket", "The secret key of the QR code does not match the key of the registrant.",
                            function() {
                                window.history.back();
                            });
                    }
        ...
        function getRegistrantFromCache(server_id, event_id, registrant_id, checkin_secret) {
            var cacheData = JSON.parse(localStorage.getItem('registrantsCache') || "{}");
            if (cacheData[server_id] && cacheData[server_id][event_id]) {
                var registrant = cacheData[server_id][event_id].find(function(registrant) {
                    return registrant.registrant_id == registrant_id && registrant.checkin_secret == checkin_secret;
                });
                if (registrant) {
                    return registrant;
                }
        ...
        function setAutoCheckIn(flag) {
            localStorage.setItem('autoCheckIn', JSON.stringify(flag));
        }
        ...
        return {
            ...
            getRegistrantsForEventFromCache: getRegistrantsForEventFromCache,
            getRegistrant: getRegistrant,
            getRegistrantFromCache: getRegistrantFromCache,
            checkIn: checkIn,
            getAutoCheckIn: getAutoCheckIn,
